...
function RevampPriority:setOutputDistributionMode(superFunc, outputFillTypeId, mode, noEventSend)
  ...
  elseif mode ~= ProductionPoint.OUTPUT_MODE.KEEP then
    printf("Error: revamp setOutputDistribution(): Undefined mode '%s'", mode)
...
function RevampPriority:distributeGoods()
  ...
  for _, farmTable in pairs(self.farmIds) do
    for i = 1, #farmTable.productionPoints do
      ...
      if distributingProdPoint.mode=="hourly" then
        if distributingProdPoint.endHour < currentHour or distributingProdPoint.startHour > currentHour then
          skip = true
        end
      end
          ...
          if amountToDistribute > 0 then
            ...
            local function distributePriority(priority, amountToDistribute, prodPointsInDemand, fillTypeIdToDistribute, distributingProdPoint)
              ...
              for n = 1, #prodPointsInDemand do
                ...
                if prodPointsInDemand[n].mode=="hourly" then
                  if prodPointsInDemand[n].endHour < currentHour or prodPointsInDemand[n].startHour > currentHour then
                    skip = true
                  end
                end
...
function ProductionPoint:setInputPriority(inputFillTypeId, priority, noEventSend)
  if self.inputFillTypeIds[inputFillTypeId] == nil then
    printf("Error: setInputPriority(): fillType '%s' is not an input fillType", g_fillTypeManager:getFillTypeNameByIndex(inputFillTypeId))
  ...
  if priority > 10 then
    printf("Error: setInputPriority(): Priority '%s' is not supported", priority)
