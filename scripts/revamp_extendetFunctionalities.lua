...
1.0.0.1 @ 30.11.2022 - AddByFillTypeStoreItems erweitert um den mod Titel für die Bauanzeige
		...
		for x = 1, #Revamp.SellFillTypes do
			...
			if SellFillType.fillType~=nil then
				...
				if fillTypeIndex == nil then
					Logging.xmlError(xmlFile, "Production Revamp: AddToSellingstation: FillType '%s' does not exist skipped. Mod: '%s'", SellFillType.fillType, SellFillType.modName)
				else
					local baseFillTypeIndex = g_fillTypeManager:getFillTypeIndexByName(SellFillType.base)
					if baseFillTypeIndex == nil then
						Logging.xmlError(xmlFile, "Production Revamp: AddToSellingstation: BaseFillType '%s' does not exist skipped. Mod: '%s'", SellFillType.base, SellFillType.modName)
					else
...
function Revamp:AddAdditionalStoreItems(xmlFilename, defaultXMLFilename)
	...
	for _, fillTypeDependentStoreItem in pairs(Revamp.AddByFillTypeStoreItems) do
		...
		if addItem then
			...
			g_storeManager:loadItem(fillTypeDependentStoreItem.xmlFilename, fillTypeDependentStoreItem.baseDirectory, fillTypeDependentStoreItem.customEnvironment, true, false, fillTypeDependentStoreItem.title)
