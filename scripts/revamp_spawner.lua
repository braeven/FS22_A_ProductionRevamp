  ...
  if ownerFarmId == AccessHandler.EVERYONE then
    ...
    if self.productionPoint.palletSpawner == nil then
        ...
        for _, mode in pairs(modes) do
          if mode =="hourly" then
            ...
            if production.endHour < currentHour or production.startHour > currentHour then
              startHour = production.startHour
              endHour = production.endHour
              skip = true
            end
          end
        end
      ...
      if not skip then
        ...
        local dummy = {}
        dummy.empty = true
        local text = string.format(g_i18n:getText("Revamp_ProductionClosed"), self.productionPoint:getName(), startHour, endHour)
        table.insert(selectableOptions, dummy)
...
function ProductionPointActivatable:fillTypeSelected(selectedOption, args, rproductionPoint)
  ...
  elseif selectedArg.closed then
    local dummy = {}
    dummy.empty = true
    local text = string.format(g_i18n:getText("Revamp_ProductionClosed"), rproductionPoint:getName(), selectedArg.startHour, selectedArg.endHour)
    table.insert(selectableOptions, dummy)
    table.insert(options, text)
    --Dialogbox erstellen
    local dialogArguments = {
  ...
  elseif selectedArg.empty then
    ...
    if #possibilities == 1 then
        ...
        elseif possibilities[1] == "woodLog" then
            self:chooseWoodLogAmount(self, rproductionPoint, selectedArg.fillLevel, currentFillType);
            return;
        else
...
function ProductionPointActivatable:spawnTypeSelected(selectedOption, args)
    ...
    elseif selectedArg.possibilitie == "woodLog" then
        self:chooseWoodLogAmount(self, selectedArg.productionPoint, selectedArg.fillLevel, selectedArg.fillType);
        return;
    else
...
function ProductionPointActivatable:woodLogsAmountSelected(selectedOption, args)
    ...
    if self.productionPoint.isServer then
        ...
        RevampSpawnWoodLogsAtProductionEvent.sendEvent(self.productionPoint, self.productionPoint.pendingWoodLogs, self.productionPoint.amountPerWoodLog, self.productionPoint.fillTypeIndex)
...
function ProductionPointActivatable:choosePalletAmount(self, rproductionPoint, availableFillLevel, fillType)
    ...
    rproductionPoint.pendingLiters = availableFillLevel
    rproductionPoint.fillTypeIndex = fillType.index
    ...
    for i=1, maxPallets do
        ...
        table.insert(options, i .. " " .. g_i18n:getText("Revamp_PalletSiloItem") .. " (" ..RevampHelper:formatVolume(math.min(amountPerPallet * i, rproductionPoint.pendingLiters), 0, fillType.unitShort) .. ")")
    end
...
function ProductionPointActivatable:amountSelected(selectedOption, args)
  ...
  if currentFillType.name == "TREESAPLINGS" then
    ...
    for index, treeSaplingType in ipairs(storeItem.configurations.treeSaplingType) do
      ...
      table.insert(selectableOptions, {treeSaplingTypeIndex=treeSaplingType.index, amount=selectedArg.amount, amountPerPallet=selectedArg.amountPerPallet, productionPoint=selectedArg.productionPoint, fillTypeIndex=selectedArg.fillTypeIndex})
      table.insert(options, title)
    end
  ...
  productionPoint.pendingLiters = math.min(productionPoint.pendingLiters, totalAmount)
  --Event aufrufen für Multiplayer
  RevampSpawnPalletsEvent.sendEvent(productionPoint, productionPoint.ownerFarmId, selectedArg.fillTypeIndex, productionPoint.pendingLiters)
end
...
function ProductionPoint:getPalletCallback(pallet, result, fillTypeIndex)
  ...
  if pallet ~= nil then
    ...
    if pallet.isBale == nil then
      ...
      delta = pallet:addFillUnitFillLevel(self.ownerFarmId, 1, self.pendingLiters, fillTypeIndex, ToolType.UNDEFINED)
    else
      --Da Ballen mit vorgegebenen Volumen spawnen, Capacity auslesen um sie aus dem Storage zu entfernen
      delta = pallet.capacity
    end

    self.pendingLiters = math.max(self.pendingLiters - delta, 0)
    ...
    if self.pendingLiters > 5 then
      ...
      if pallet.isBale and self.pendingLiters < pallet.capacity then
        pallet.capacity = self.pendingLiters
      end
...
function ProductionPoint:TimerCallback()
  if self.isbale == nil then
    self.palletSpawner:spawnPallet(self.ownerFarmId, self.fillTypeIndex, ProductionPoint.getPalletCallback, self)
  else
    local bale = self.bale
    self.palletSpawner:spawnBale(self.ownerFarmId, self.fillTypeIndex, ProductionPoint.getPalletCallback, self, bale.isBale, bale.isRoundbale, bale.width, bale.height, bale.length, bale.diameter, bale.capacity, bale.wrapState, bale.customEnvironment, self.wrapColor)
  end
end
...
function ProductionPoint:updatePallets(bale)
  if self.isServer then
    if not self.spawnPending and self.pendingLiters > 5 then
...
function ProductionPointActivatable:baleSelected(selectedOption, args)
  ...
  --Werte für spawner hinterlegen
  productionPoint.fillTypeIndex = fillTypeIndex
  productionPoint.pendingLiters = selectedArg.fillLevel
  ...
  for i=1, maxBales do
    ...
    table.insert(options, i .. " " .. g_i18n:getText("Revamp_BaleSiloItem") .. " (" ..RevampHelper:formatVolume(math.min(amountPerBale*i, productionPoint.pendingLiters), 0, currentFillType.unitShort) .. ")");
  end
...
function ProductionPointActivatable:spawnBales(selectedOption, args)
  ...
  productionPoint.pendingLiters = math.min(productionPoint.pendingLiters, totalAmount)
	...
	--Damit das gesammt FillVolume ausgelager werden kann, überprüfen ob der Ballen größer ist als der restliche Inhalt und anpassen
  if productionPoint.pendingLiters < selectedArg.baleSize.capacity then
	  productionPoint.capacity = productionPoint.pendingLiters
	else
	  productionPoint.capacity = selectedArg.baleSize.capacity
  end
  RevampSpawnBalesEvent.sendEvent(productionPoint, productionPoint.ownerFarmId, selectedArg.fillTypeIndex, productionPoint.pendingLiters, true, baleSize.isRoundbale, baleSize.width, baleSize.height, baleSize.length, baleSize.diameter, productionPoint.capacity, baleSize.wrapState, baleSize.customEnvironment)
...
function PalletSpawner:spawnBale(farmId, fillTypeId, callback, callbackTarget, isBale, isRoundbale, width, height, length, diameter, capacity, wrapState, customEnvironment, wrapColor)
  ...
  if pallet ~= nil then
    ...
    Logging.devError("RevampSpawner: no bale for fillTypeId", fillTypeId)
...
function InGameMenuProductionFrame:menuconnector()
  ...
  if skip then
    ...
    closed.closed = true
    closed.startHour = startHour
    closed.endHour = endHour
